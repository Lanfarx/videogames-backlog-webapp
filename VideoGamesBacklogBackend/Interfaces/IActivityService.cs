...
namespace VideoGamesBacklogBackend.Interfaces
{
    public interface IActivityService
    {
        ...
        Task<List<ActivityDto>> GetRecentActivitiesAsync(int userId, int count = 10);        
        Task<List<ActivityDto>> GetActivitiesByGameAsync(int gameId, int userId);
        Task<Dictionary<string, int>> GetActivityStatsByTypeAsync(int userId, int? year = null);          // Metodi helper per la creazione di attività specifiche
